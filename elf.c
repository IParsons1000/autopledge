...
elf_t *elf_load(char *file){
	...
	if(elf == NULL){
		log_error("malloc(sizeof(elf_t)) failed\n");
		return NULL;
	};

	elf->name = malloc(strlen(file));
	if(elf->name == NULL){
		log_error("malloc(strlen(file)) failed\n");
	};

	elf->name = strncpy(elf->name, file, strlen(file));
	if(elf->name == NULL){
		log_error("strncpy(elf->name, file, strlen(file)) failed\n");
	};

	/* read elf header into memory */

	if(read(fd, &elf->ehdr, sizeof(Elf64_Ehdr)) != sizeof(Elf64_Ehdr)){
		log_error("unable to load elf header of file %s\n", file);
		elf_free(elf);
		return NULL;
	};
	...
	if(!( elf->ehdr.e_ident[0] == ELFMAG0 && elf->ehdr.e_ident[1] == ELFMAG1 && elf->ehdr.e_ident[2] == ELFMAG2 && elf->ehdr.e_ident[3] == ELFMAG3 )){
		log_error("file %s is not in elf format\n", file);
		elf_free(elf);
		return NULL;
	};
	...
	if(!elf->ehdr.e_phnum){
		...
		if(elf->phdr == NULL){
			log_error("malloc(elf->ehdr.e_phnum * elf->ehdr.e_phentsize) failed\n");
			elf_free(elf);
			return NULL;
		};

		if(lseek(fd, elf->ehdr.e_phoff, SEEK_SET) == -1){
			log_error("unable to load program header(s) of file %s\n", file);
			elf_free(elf);
			return NULL;
		};
		...
		for(int i = 0; i < elf->ehdr.e_phnum; i++){
			if(read(fd, &elf->phdr[i], sizeof(Elf64_Phdr)) != sizeof(Elf64_Phdr)){
				log_error("unable to read program header(s) of file %s\n", file);
				elf_free(elf);
				return NULL;
			};
		};
	...
	if(elf->phdr == NULL){
		...
		if(elf->segs == NULL){
			log_error("malloc(elf->ehdr.e_phnum * sizeof(char *)) failed\n");
			elf_free(elf);
			return NULL;
		};
	...
	if(!elf->ehdr.e_shnum){
		...
		if(elf->shdr == NULL){
			log_error("malloc(elf->ehdr.e_shnum * elf->ehdr.e_shentsize) failed\n");
			elf_free(elf);
			return NULL;
		};

		if(lseek(fd, elf->ehdr.e_shoff, SEEK_SET) == -1){
			log_error("unable to load section header(s) of file %s\n", file);
			elf_free(elf);
			return NULL;
		};
		...
		for(int i = 0; i < elf->ehdr.e_shnum; i++){
			if(read(fd, &elf->shdr[i], sizeof(Elf64_Shdr)) != sizeof(Elf64_Shdr)){
				log_error("unable to read section header(s) of file %s\n", file);
				elf_free(elf);
				return NULL;
			};
		};
	...
	if(elf->shdr == NULL){
		...
		if(elf->secs == NULL){
			log_error("malloc(elf->ehdr.e_shnum * sizeof(char *)) failed\n");
			elf_free(elf);
			return NULL;
		};
...
char **elf_get_dynsym(elf_t *elf){
	...
	if(dsymi == -1){
		log_error("no .dynsym section in file %s\n", elf->name);
		return NULL;
	};

	if(dstri == -1){
		log_error("no .dynstr section in file %s\n", elf->name);
		return NULL;
	};
	...
	if(elf->shdr[dsymi].sh_entsize != sizeof(Elf64_Sym)){
		log_error(".dynsym entry size does not match expected value in file %s\n", elf->name);
		return NULL;
	};
	...
	for(int j = 0; j < (int)(elf->shdr[dsymi].sh_size / elf->shdr[dsymi].sh_entsize); j++){
		if(dynsym[j].st_info & STT_FUNC){
			...
			if(syms == NULL){
				log_error("realloc(syms, numsyms * sizeof(char *)) failed\n");
				return NULL;
			};
	...
	if(syms == NULL){
		log_error("realloc(syms, (numsyms + 1) * sizeof(char *)) failed\n");
		return NULL;
	};
	syms[numsyms] = NULL;
